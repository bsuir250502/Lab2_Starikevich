...
int main(int argc, char **argv)
{
	...
	for(i=0; i<students_num; i++) students[i].term = term;
...
char myget_arg(int argc, char **argv)
{
	if(argc==2)
	{
		if(!(strcmp(argv[1], "-t1"))) return '1';
		if(!(strcmp(argv[1], "-t2"))) return '2';
		if(!(strcmp(argv[1], "-t3"))) return '3';
		if(!(strcmp(argv[1], "-t4"))) return '4';
...
int *choose_term_array(student_T *students, int i)
{
	...
	{
		case 1: term = students[i].sessions.term1;
		break;

		case 2: term = students[i].sessions.term2;
		break;

		case 3: term = students[i].sessions.term3;
		break;

		case 4: term = students[i].sessions.term4;
		break;
	}
...
{	
	...
	for(i=0; i<students_num; i++)
	{
		printf("\n(Student: #%d, %s %s %s):\n", i+1, students[i].names.name, students[i].names.surname, students[i].names.middlename);
		term = choose_term_array(students, i);
		for(exam=0; exam<exams_num; exam++)
		{
			printf("Enter the result of subject '%s' [1-10]: ", subject_list(exam));	
...
char *subject_list(int exam)
{
	char *list[] = {"Math", "Physics", "Programming", "Engineering", "English"};	
